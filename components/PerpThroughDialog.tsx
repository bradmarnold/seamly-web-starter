...
export default function PerpThroughDialog({ open, onClose }:{ open:boolean; onClose:()=>void }){
  const { points, addPointPerpThrough } = useDraftStore(s=>s);
  ...
  const submit = () => {
    ...
    addPointPerpThrough(through, refA, refB, name.trim(), L);
    onClose();
  };

  return (
    <Modal open={open} title="Perpendicular through point" onClose={onClose}>
