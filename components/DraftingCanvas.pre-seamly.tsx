...
export type ToolMode = 'pan'|'move'|'lineBetween'|'measure'|'pointLA'|'splinePath';
...
export function DraftingCanvas({ mode, zoomSignal, onRequestEdit, onPointLARequest }: Props) {
  ...
  const { points, lines, segs, updatePoint, addLineBetween, addSpline } = useDraftStore(s => s);
  ...
  // Spline anchors being collected
  ...
  useEffect(() => {
    ...
    else if (mode === 'splinePath') setHint(splineAnchors.length ? 'Spline: click next anchor (Enter to finish, Esc cancel)' : 'Spline: click first anchor point');
    else setHint('');
  }, [mode, splineAnchors.length]);
  ...
  // segments as lines for snapping helpers
  ...
  const onPointerDown = (e: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (effectiveMode === 'lineBetween') {
      ...
      if (hit !== -1) {
        ...
        else if (pickA !== hit) { addLineBetween(pickA, hit); setPickA(hit); setHint('Click next point (Esc to finish)'); }
    ...
    if (effectiveMode === 'splinePath') {
      // only accept existing points as anchors for now
      const hit = findClosestPointPx(p, 28);
      if (hit !== -1) {
        setSplineAnchors(prev => [...prev, hit]);
        setHint('Spline: click next anchor (Enter to finish, Esc cancel)');
      } else {
        setHint('Spline: click a point to add as anchor');
      }
      return;
    }
  };
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    ...
    if (e.key === 'Escape') {
      setDragId(null); setPickA(null); setMeasureA(null); setMeasureB(null); setBaseLA(null);
      if (splineAnchors.length) setSplineAnchors([]);
      setHint('');
    }
    ...
    if (effectiveMode==='splinePath' && e.key === 'Enter' && splineAnchors.length >= 2) {
      addSpline(splineAnchors, false);
      setSplineAnchors([]);
      setHint('Spline created');
      e.preventDefault();
    }
  };
  ...
  // Build a temporary path string to preview spline
  const splinePreviewPath = useMemo(()=>{
    ...
    const head = pts.length ? `M ${pts[0].x} ${pts[0].y}` : '';
    const rest = pts.slice(1).map(p=>`L ${p.x} ${p.y}`).join(' ');
    const tail = (snap?.tag==='PT') ? ` L ${snap.x} ${snap.y}` : '';
    return `${head} ${rest}${tail}`;
  }, [effectiveMode, splineAnchors, points, snap?.x, snap?.y, snap?.tag]);
  ...
  return (
    ...
    >
      ...
      {/* Midpoint & Intersection snap hints */}
      ...
      {(effectiveMode==='pointLA' && baseLA!=null) ? (() => {
        const A = points.find(p=>p.id===baseLA)!; const rawB = snap ?? cursor; const B = shiftDown ? snapAngle45(A, rawB) : rawB;
        const d = dist(A,B), ang = angleDeg(A,B);
        return (
          <g>
      ...
      {/* Spline preview */}
      {(effectiveMode==='splinePath' && splineAnchors.length>0 && splinePreviewPath) ? (
        <path d={splinePreviewPath} fill="none" stroke="#059669" strokeDasharray="6 4" strokeWidth={1.6}/>
      ) : null}

      {/* Snap options pill */}
      <g>
        <rect x={viewBox.x+10} y={viewBox.y+44} width="360" height="28" rx="6" ry="6" fill="#ffffff" stroke="#cbd5e1"/>
        <foreignObject x={viewBox.x+16} y={viewBox.y+46} width="348" height="24">
          <div xmlns="http://www.w3.org/1999/xhtml"
               style={{
                 fontFamily:'system-ui, -apple-system, Segoe UI, Roboto, sans-serif',
                 fontSize:'12px', color:'#334155', display:'flex',
            ...
            <label style={{display:'flex', gap:'4px', alignItems:'center'}}>
              <input type="checkbox" checked={snapMids} onChange={()=>setSnapMids(v=>!v)} />
              <span>Midpoints</span>
            </label>
            <label style={{display:'flex', gap:'4px', alignItems:'center'}}>
              <input type="checkbox" checked={snapInters} onChange={()=>setSnapInters(v=>!v)} />
              <span>Intersections</span>
            </label>
            <span style={{marginLeft:'auto', opacity:.6}}>Keys: 1 Pts, 2 Mid, 3 Int, Space Pan</span>
          </div>
        </foreignObject>
      </g>
      ...
      {hint && (
        <g>
          <rect x={viewBox.x+10} y={viewBox.y+10} width="560" height="26" rx="6" ry="6" fill="#ffffff" stroke="#cbd5e1"/>
      ...
      {/* Status bar */}
      <g>
        <rect x={viewBox.x+10} y={viewBox.y+viewBox.h-34} width="720" height="24" rx="6" ry="6" fill="#ffffff" stroke="#cbd5e1"/>
        <text x={viewBox.x+24} y={viewBox.y+viewBox.h-18} fill="#334155" fontSize="12">
          ...
          {tempPan ? ' · (Space: Pan)' : ''}
          {snap?.tag ? ` · ${snap.tag}` : ''}
          {splineAnchors.length ? ` · Anchors ${splineAnchors.length}` : ''}
        </text>
      </g>
    </svg>
  );
