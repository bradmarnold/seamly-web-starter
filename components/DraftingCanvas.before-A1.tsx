...
type Props = {
  mode: ToolMode;
  zoomSignal?: { version:number; dir: 1|-1 };
};

export function DraftingCanvas({ mode, zoomSignal }: Props) {
    ...
    >
      ...
      {points.map((p) => (
        <g key={p.id}>
          ...
          {pickA === p.id ? <circle cx={p.x} cy={p.y} r={8} fill="none" stroke="#3aa0ff" strokeWidth={1.5} /> : null}
        </g>
      ))}
    </svg>
  );
