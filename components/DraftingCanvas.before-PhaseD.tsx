...
export function DraftingCanvas({ mode, zoomSignal, onRequestEdit }: Props) {
  ...
  function arcPath(center:{x:number;y:number}, start:{x:number;y:number}, end:{x:number;y:number}, ccw:boolean){
    const r = Math.hypot(start.x-center.x, start.y-center.y) || 0.0001;
    // Compute sweep and large-arc flags from direction
    ...
    >
      ...
      {segs.map((s) => {
        if (s.kind === 'line') {
          ...
          const d = arcPath(c, a, b, s.ccw);
          return <path key={`seg-${s.id}`} d={d} fill="none" stroke="#ffb86b" strokeWidth={2}/>;
        }
      })}

      {/* Legacy: draw old "lines" too (kept for compatibility) */}
      {lines.map((ln, i) => {
