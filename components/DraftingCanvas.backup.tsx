...
export function DraftingCanvas({ mode, zoomSignal, onRequestEdit }: Props) {
  ...
  const [shiftDown, setShiftDown] = useState(false); // updated from mouse events' shiftKey
  ...
  const findClosestPointPx = (p:{x:number;y:number}, pixelThresh=24) => {
  ...
  const onMouseDown = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (mode === 'lineBetween') {
      ...
      if (hit !== -1) {
        ...
        // click empty canvas: cancel preview if any
        if (pickA != null) setPickA(null);
      }
      return;
    }
  ...
  const onKeyDown = (e: React.KeyboardEvent) => {
    if (e.key === 'Escape') {
      setDragId(null); setPickA(null); setMeasureA(null); setMeasureB(null);
    }
  ...
  return (
    <svg
      ...
      style={{ width: '100%', height: '100%', display: 'block', background:'#ffffff', cursor: mode === 'pan' ? 'grab' : 'crosshair' }}
    >
      {/* Segments */}
      {segs.map((s) => {
        if (s.kind === 'line') {
      ...
      {/* Rubber-band preview when Line tool has first point */}
      {(mode==='lineBetween' && pickA!=null) ? (() => {
        ...
        return (
          <g>
            ...
            <text x={B.x+8} y={B.y-8} fill="#60a5fa" fontSize="12" paintOrder="stroke" stroke="#0c1018" strokeWidth="2">
