...
/** Constructions (provenance) */
export type ConstrLA = { kind:'LA'; base:number; length:number; angle:number };              // length/angle from base
export type ConstrPOL = { kind:'POL'; a:number; b:number; dist:number };                     // point on line AB at distance from A
export type Constraint = ConstrLA | ConstrPOL;
...
type DraftState = {
  ...
  // edit constructions
  editPointLA: (id:number, opts:{ length?:number; angle?:number; base?:number }) => void;
  editPointPOL: (id:number, opts:{ dist?:number; a?:number; b?:number }) => void;
...
export const useDraftStore = create<DraftState>((set, get) => ({
  ...
  addPointOnLineDistance: (aId, bId, name, dist) => {
    ...
    const pt: Pt = { id: nextPointId++, name, x, y, constraint: { kind:'POL', a:aId, b:bId, dist } };
    set(state => ({
      points: [...state.points, pt],
      blocks: state.blocks.map(b => b.id === state.currentBlockId ? { ...b, pointIds: [...b.pointIds, pt.id] } : b)
    }));
  ...
  /** EDIT: change length/angle and/or base, then recompute absolute coords */
  editPointLA: (id, opts) => set(state => {
    const idx = state.points.findIndex(p=>p.id===id);
    if (idx === -1) return state as any;
    const pt = state.points[idx];
    const c = pt.constraint;
    if (!c || c.kind!=='LA') return state as any;

    const baseId = opts.base ?? c.base;
    if (baseId === id) return state as any; // avoid self-reference
    const base = state.points.find(p=>p.id===baseId);
    if (!base) return state as any;

    const length = opts.length ?? c.length;
    const angle  = opts.angle  ?? c.angle;
    const rad = angle * Math.PI / 180;
    const x = base.x + length * Math.cos(rad);
    const y = base.y + length * Math.sin(rad);

    const nextPts = state.points.slice();
    nextPts[idx] = { ...pt, x, y, constraint: { kind:'LA', base: baseId, length, angle } };
    return { points: nextPts };
  }),

  /** EDIT: change distance and/or line (A,B) then recompute absolute coords */
  editPointPOL: (id, opts) => set(state => {
    const idx = state.points.findIndex(p=>p.id===id);
    if (idx === -1) return state as any;
    const pt = state.points[idx];
    const c = pt.constraint;
    if (!c || c.kind!=='POL') return state as any;

    const aId = opts.a ?? c.a;
    const bId = opts.b ?? c.b;
    if (aId === bId) return state as any;

    const A = state.points.find(p=>p.id===aId);
    const B = state.points.find(p=>p.id===bId);
    if (!A || !B) return state as any;

    const dist = opts.dist ?? c.dist;
    const d = distance(A.x,A.y,B.x,B.y);
    const t = d === 0 ? 0 : dist / d;
    const x = A.x + (B.x - A.x) * t;
    const y = A.y + (B.y - A.y) * t;
    ...
    const nextPts = state.points.slice();
    nextPts[idx] = { ...pt, x, y, constraint: { kind:'POL', a:aId, b:bId, dist } };
    return { points: nextPts };
  }),
  ...
  addLineBetween: (aId, bId) => {
    ...
    set(state => ({
      segs: [...state.segs, seg],
      lines: [...state.lines, { a:aId, b:bId }] // legacy
    }));
    return seg.id;
  },
