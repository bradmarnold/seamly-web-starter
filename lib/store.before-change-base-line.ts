...
type DraftState = {
  ...
  // edit constructions
  editPointLA: (id:number, opts:{ length?:number; angle?:number }) => void;
  editPointPOL: (id:number, opts:{ dist?:number }) => void;
...
export const useDraftStore = create<DraftState>((set, get) => ({
  ...
  /** EDIT: change length/angle and recompute absolute coords */
  editPointLA: (id, opts) => set(state => {
    ...
    const base = state.points.find(p=>p.id===c.base);
    ...
    nextPts[idx] = { ...pt, x, y, constraint: { ...c, length, angle } };
    return { points: nextPts };
  }),

  /** EDIT: change distance along AB and recompute absolute coords */
  editPointPOL: (id, opts) => set(state => {
    ...
    const A = state.points.find(p=>p.id===c.a);
    const B = state.points.find(p=>p.id===c.b);
    ...
    const nextPts = state.points.slice();
    nextPts[idx] = { ...pt, x, y, constraint: { ...c, dist } };
    return { points: nextPts };
  }),
