...
import BulkMeasurementsDialog from '@/components/BulkMeasurementsDialog';
...
export default function MeasurementsPage(){
  ...
  const onBulkApply = (entries: Record<string, number>) => {
    if(!selected) return;
    const merged = { ...selected.vars, ...entries };
    const file = { ...selected, vars: merged };
    const next = { ...db, files: db.files.map(f=> f.id===file.id ? file : f) };
    setDb(next); saveDB(next);
    setShowBulk(false);
  };
